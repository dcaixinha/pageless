defmodule PagelessWeb.UserAuth do
  ...
  @remember_me_cookie "_pageless_web_user_remember_me"
  @remember_me_options [
    sign: true,
  ...
  def log_out_user(conn) do
    ...
    if live_socket_id = get_session(conn, :live_socket_id) do
      PagelessWeb.Endpoint.broadcast(live_socket_id, "disconnect", %{})
    end
  ...
  def disconnect_sessions(tokens) do
    Enum.each(tokens, fn %{token: token} ->
      PagelessWeb.Endpoint.broadcast(user_session_topic(token), "disconnect", %{})
    end)
  end
  ...
  the `current_scope`:
      ...
      defmodule PagelessWeb.PageLive do
  ...
  Or use the `live_session` of your router to invoke the on_mount callback:
      ...
      live_session :authenticated, on_mount: [{PagelessWeb.UserAuth, :require_authenticated}] do
  ...
  def on_mount(:require_admin, _params, session, socket) do
    ...
    if user && Pageless.Accounts.User.admin?(user) do
      {:cont, socket}
    else
      socket =
        socket
